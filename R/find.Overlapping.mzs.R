...
#' 
#' This function matches features between two datasets using the following user
#' defined criteria: 1) Maximum m/z difference (+/-) ppm 2) Maximum retention
#' time difference in seconds
#' 
#' The find.Overlapping.mzs function operates on two sets of feature lists with
#' m/z and retention times for each feature, denoted by L1 and L2, and iterates
#' over all m/z values in L1 to find those that are within a user defined m/z
#' (ppm) and retention time (sec) threshold in L2. Optionally, the user can
#' match features based on only the m/z values by setting time.thresh=NA. The
#' find.Unique.mzs function uses a similar algorithm to find unique features
#' that are not within a user defined mass and retention time threshold level.
#' 
#' @param dataA apLCMS or XCMS feature table for dataset A
#' @param dataB apLCMS or XCMS feature table for dataset B
#' @param mz.thresh Maximum m/z difference (+/-) ppm. eg: 10
#' @param time.thresh Maximum retention time difference (+/-) secs. eg: 300
...
#' @return Matrix of overlapping features with columns: index.data.A: index of
#' overlapping m/z in dataset A mz.data.A: m/z in dataset A time.data.A:
#' retention time in dataset A index.data.B: index of overlapping m/z in
#' dataset B mz.data.B: m/z in dataset B time.data.B: retention time in dataset
#' B
#' @author Karan Uppal <kuppal2@@emory.edu>
#' @seealso apLCMS.align, XCMS.align, find.Unique.mzs, getVenn
#' @keywords ~overlap ~common
    ...
    mz_groups <- lapply(1:dim(data_a)[1], function(j) {
        ...
        if (is.na(time.thresh) == FALSE) {
            if (length(getbind_same) > 0) {
                ...
                for (comindex in 1:length(getbind_same)) {
                  tempA = cbind(j, data_a[j, c(1, 
                    2)])
                  tempB = cbind(getbind_same[comindex], 
                    data_b[getbind_same[comindex], 
                      c(1, 2)])
            ...
            if (length(getbind_same) > 0) {
                ...
                rnames <- paste("mz", j, "_", seq(1, 
                  length(getbind_same)), sep = "")
                rownames(commat) = rnames
