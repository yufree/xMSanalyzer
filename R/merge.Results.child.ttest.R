merge.Results.child.ttest <- function(dataA, max.mz.diff = 15, 
    max.rt.diff = 300, merge.eval.pvalue = 0.05, alignment.tool = "apLCMS", 
    mult.test.cor = FALSE, mergecorthresh = 0.7, col.rm.index = NA, 
    missingvalue = 0) {
    diff_mz_num = 1
    unique_mz = {
    }
    ...
    compare_intensities_ttest <- function(other_feats, 
        y, merge.eval.pvalue, mergecorthresh = 0.6) {
            ...
            if (length(x) > 1 & length(y) > 1) {
                ...
                if (num_com < 1) {
                  ...
                  if (log2fc > 0.1) {
                    ...
                    ttest_res = try(t.test(x, y, paired = TRUE, 
                      na.omit = TRUE), silent = TRUE)
                    ...
                    if (is(ttest_res, "try-error")) {
                      ...
                      ttest_res2 = try(t.test(x, y, 
                        paired = FALSE, na.omit = TRUE), 
                        silent = TRUE)
                    ...
                    ttest_pval <- max(ttest_pval, 
                      ttest_pval2, na.rm = TRUE)
                    
                    if (length(x) > 2 & length(y) > 
                      2) {
                      cortest_res = try(cor.test(as.numeric(x), 
                        as.numeric(y), method = "spearman", 
    ...
    fname <- paste("mz_groups", max.mz.diff, ".Rda", 
        sep = "")
    ...
    diffmatres <- lapply(1:length(mz_groups), function(j) {
        fname <- paste("mz_groups", max.mz.diff, "_", 
            j, ".Rda", sep = "")
        ...
        if (dim(tempdata)[1] > 1) {
            ...
            tempdata <- tempdata[order(tempdata$Qscore, 
                decreasing = TRUE), ]
            ...
            for (d in 1:dim(tempdata)[1]) {
                ...
                if (d %in% dup_ind == FALSE & d %in% 
                  ...
                  if (length(getbind_rtsame) > 1) {
                    ...
                    y = cur_feat[c(2, sample.col.start:(dim(tempdata)[2] - 
                      8))]
                    
                    diff_rt <- abs(other_feats[, 2] - 
                      y[2])
                    
                    ttest_sum <- compare_intensities_ttest(other_feats[, 
                      c(2, sample.col.start:(dim(tempdata)[2] - 
                        8))], y, merge.eval.pvalue, 
                      mergecorthresh)
                    ...
                    curdupnames <- which(allrownames %in% 
                      curnames)
                    
                    curnames <- curnames[-which(curnames == 
                      rowname)]
                    
                    
                    
                    if (length(which(curdupnames %in% 
                      dup_ind == TRUE)) < 1) {
                      ...
                      if (length(same_feat_ind) > 
                        ...
                        maxint <- log((maxint + 1), 
                          10)
                        maxint <- maxint + 0.001
                        maxint <- maxint/max(maxint, 
                          na.rm = TRUE)
                        
                        commat_qscore <- as.numeric(commat$Qscore) * 
                          maxint  #cv_range/as.numeric(commat$numgoodsamples)*as.numeric(deltappm_res)
                        # inverse of the actual qscore
                        best_level_index = which(as.numeric(commat_qscore) == 
                          max(as.numeric(commat_qscore), 
                            na.rm = TRUE))
